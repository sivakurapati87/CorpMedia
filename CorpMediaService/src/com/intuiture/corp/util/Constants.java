package com.intuiture.corp.util;

public class Constants {
	public static final String COMPANYJSONFILE = "\\companies.txt";
	public static final String BANKJSONFILE = "\\banks.txt";
	public static final String SIGNATORSJSONFILE = "\\signators.txt";
	public static final String STATESJSONFILE = "\\states.txt";
	public static final String companyLeavePlanId = "companyLeavePlanId";
	public static final String FILEUPLOADEDPATH = "\\CorpMediaFiles\\CompanyLogos";
	public static final String MONTHS = "MONTHS";
	public static final Integer ENDDAY = 100;
	public static final Double BASIC = 0.4d;
	public static final Double HRA = 0.5d;
	public static final Double PF = 0.048d;
}
